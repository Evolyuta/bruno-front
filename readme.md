...
```bash
# Linux/macOS — через скрипт запуска (обрабатывает переменные окружения VS Code)
./start-dev.sh

# Или вручную (обязательно при запуске из терминала VS Code)
